...
import AllergiesModal from './AllergiesModal'; // Importez le composant AllergiesModal depuis son emplacement correct
...
const Profile = () => {
  ...
  const handleUploadImage = async (event) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/user/updateImage`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
  ...
  return (
    <>
      ...
      <section style={{ backgroundImage: `url(${backgroundImage})`, minHeight: "100vh", padding: "20px" }}>
      <Navs />
            <div>
              <Toaster />
            </div>
